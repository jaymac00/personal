...
function altsToggle() {
	if (alts) {
		event.target.innerHTML = "Alts: OFF";
		alts = false;
	} else {
		event.target.innerHTML = "Alts: ON";
		alts = true;
	}
}
/* toggle alts */
...
function miiToggle() {
	if (miis) {
		event.target.innerHTML = "Mii Moveset: OFF";
		miis = false;
	} else {
		event.target.innerHTML = "Mii Moveset: ON";
		miis = true;
	}
}
/* toggle mii moveset */
...
function randAlt() {
	let alt = Math.floor(Math.random() * 8);
	for (let i = 0; i < roster.length; ++i) {
...
function randMii() {
	let mii = "" + (Math.floor(Math.random() * 3) + 1)
...
function randFe() {
	if (event.target.id == "ironman") {
		...
		if (remaining == 1) {
			...
			document.getElementById("random").innerHTML = "<img src=\"./assets/fighters/50px-CompleteHeadSSBU.png\"><br>Congratulations! You did it!";
			++success;
			document.getElementById("remaining").innerHTML = "Fighters Remaining: " + remaining;
			document.getElementById("success").innerHTML = "Current Run: " + success;
			--remaining;
